"""
Sector screening: compare current price to 5y and 10y highs. Includes debug
output to diagnose failures.
"""
import pandas as pd
...
def sector_screen(etf_ticker, min_years=5, debug=True):
    ...
    If debug=True, print the computed values.
    """
    # fetch daily data as far back as possible
    rows = data_fetcher.fetch_price_history(etf_ticker)
    if len(rows) < 252 * 2:  # at least 2 years (to have some data)
        if debug:
    ...
    if len(s_5y) < 252:  # less than a year of data in 5y window
        if debug:
    ...
    if len(s_10y) < 252:  # less than a year in 10y window
        if debug:
            print(f"  DEBUG: {etf_ticker} – insufficient data in 10y window: {len(s_10y)} rows")
        # still compute if we have some data
    ...
    if debug:
        print(f"  DEBUG: {etf_ticker} – Current: {current:.2f}, 5y high: {high_5y:.2f} ({stats['pct_of_5y_high']*100:.1f}%), 10y high: {high_10y if high_10y else 'N/A'}, pass_5y: {pass_5y}, pass_10y: {pass_10y}")
